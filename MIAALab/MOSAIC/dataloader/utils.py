...
def data_augmentation(patch, mask_labels, apply_elastic_transform=False, allow_rotate=True):
...
def preprocess_func(patch, mask_labels, augmentation=True,
                    apply_elastic_transform=False, allow_rotate=True):
    # augmentation
    if augmentation:
        patch, mask_labels = data_augmentation(
            patch, mask_labels,
            apply_elastic_transform=apply_elastic_transform,
...
def data_preprocess(patch, mask_labels, training=True, dataset='LIDC'):
    is_lung = dataset in ('LungNodule',)
    allow_rotate = not is_lung

    patch_list, mask_labels_list = [], []
    assert (patch.shape[0] == mask_labels[0].shape[0])
    batch_num = patch.shape[0]
    for i in range(batch_num):
        patch_list.append(np.clip(patch[i, ...].numpy(), 0, 255).astype('uint8'))
        m = []
    ...
    if not training:
        assert batch_num == 1
        p, m_list = preprocess_func(patch_list[0], mask_labels_list[0],
                                    augmentation=False, allow_rotate=allow_rotate)
        m_list = ranking(m_list, is_rank=True)

        # per-image z-score (eval path)
        if is_lung:
            p = _zscore(p)
        return p, m_list
    ...
    for i in range(batch_num):
        p, m_list = preprocess_func(copy.deepcopy(patch_list[i]),
...
def _zscore(patch):
    """Per-image z-score normalization. patch shape: (B, C, H, W) or (1, C, H, W)."""
    b = patch.shape[0]
    flat = patch.view(b, -1)
    mean = flat.mean(dim=1).view(b, 1, 1, 1)
    std = flat.std(dim=1).clamp(min=1e-6).view(b, 1, 1, 1)
