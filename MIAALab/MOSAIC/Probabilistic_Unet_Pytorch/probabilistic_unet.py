...
class ProbabilisticUnet(nn.Module):
    ...
    def __init__(self, input_channels=1, num_classes=1, num_filters=[16,32,64,128,256],
                 latent_dim=6, no_convs_fcomb=4, alpha=10.0, reg_factor=1.0, original_backbone=False):
    ...
    def elbo(self, segm, loss_fct, analytic_kl=True, reconstruct_posterior_mean=False):
        ...
        return -(self.reconstruction_loss + self.alpha * self.kl)
    ...
    def val_step(self, images, sample_num=10):
        # for training don't use softmax because it is integrated in loss function
        self.forward(images, None, training=False)
        y_pred = []
        for i in range(sample_num):
            z_prior = self.prior_latent_space.sample()
            y_pred_case = self.fcomb.forward(self.unet_features, z_prior, use_softmax=False)
